...
function game() {
  update();
  updateScore(); // Вызываем функцию обновления счета
  render();
...
function endGame() {
  alert("Game Over! Вернутся в меню..");
  const scoreEvent = new CustomEvent("scoreUpdated", { detail: { score } });
...
function render() {
  ...
  //рисуем фон
  context.drawImage(fon, 0, 0, 900, 600);
  //рисуем пули
  ...
  for (i in aster) {
    ...
    context.font = "20px Arial"; // Установка шрифта и размера
